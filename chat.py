...
# try loading existing, otherwise build fresh
try:
...
def handle_volunteer(q):
    ...
    if any(w in q for w in ["uvh", "ethics", "mindfulness"]):
        return "He volunteered in the UHV Cell (2018–19), organizing sessions on ethics, empathy, and mindfulness."
    if "ieee" in q:
        return "He was an IEEE member (2017–18), attending workshops and tech talks to stay connected beyond class."
    return None
...
def handle_behavioral(q):
    ...
    if any(w in q for w in ["leadership", "team", "collaborate"]):
        return "He’s led cross-functional teams at Fagron and KLA, aligning product, QA, and engineering to deliver on time."
    if any(w in q for w in ["challenge", "problem"]):
        return "He faced a challenge optimizing Spark jobs at KLA—rewrote complex joins and used partition pruning to cut runtime by 60%."
    return None
